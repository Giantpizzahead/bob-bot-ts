...
import {
    ...
    VoiceConnectionStatus, VoiceReceiver, EndBehaviorType, StreamType, AudioPlayerStatus,
...
async function sendMessage(msgToSend, channel) {
    ...
    while (i < msgToSend.length) {
        ...
        await new Promise((resolve, reject) => setTimeout(async () => {
            ...
            if (currMessageCount === messageCounts[channel.id]) {  // Only send if message is not outdated
                try {
...
async function periodicUpdate() {
    ...
    if (false && lastMessage && Math.random() < 0.004) {
...
client.on('messageCreate', async (message) => {
    ...
    const currentUTCHour = currentTime.getUTCHours();
    const dayOfWeek = currentTime.getDay();
    if (false && currentUTCHour >= 13 && currentUTCHour < 21 && dayOfWeek >= 1 && dayOfWeek <= 5) {
        // Work hours
    ...
    if ((currMode === '20q' || currMode === 'akinator' || currMode === 'jeopardy' || currMode === 'uncensored') && message.author.bot) return;
