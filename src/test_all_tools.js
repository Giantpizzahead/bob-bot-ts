...
const imageSearchTool = tool(
    ...
    {
        name: "image-search",
        description: "An image search engine. Useful for when you need to find or want to react with an image. Input should be a search query.",
...
const fetchWebpageSchema = z.object({
    ...
    prompt: z
    .string()
    .describe("A prompt for the LLM, which will read the text on the webpage and attempt to answer the prompt based on the content it finds.")
});
...
const fetchWebpageTool = tool(
    ...
    {
        name: "fetch-webpage",
        description: "Can answer a prompt about the contents of a webpage at a given URL. Useful for finding and collecting specific info.",
...
const fetchYouTubeSchema = z.object({
    ...
    language: z
        .string()
        .optional()
        .describe("The language code for the transcript. Defaults to 'en'."),
});
...
const llmWithTools = llm.bindTools([webSearchTool, imageSearchTool, fetchWebpageTool, fetchYouTubeTool, calculatorTool, imageGenTool]);
